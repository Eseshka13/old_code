...
class ProductApi extends synchApi
{
    ...
    public function getByID($id = 0)
    {
        ...
        $db_res = \Bitrix\Catalog\ProductTable::getList($filter);
        $result = $db_res->fetchAll();
        if (!empty($result)) $result[0]['PRICE'] = \Bitrix\Catalog\PriceTable::getList(['filter' => ['PRODUCT_ID' => $result[0]['ID']]])->fetchAll();
    ...
    function GetList($fName = 'ACTIVE', $fVal = 'Y')
    {
        ...
        $filter['select'] = [
            ...
            'CODE' => 'IBLOCK_ELEMENT.CODE',
            'TYPE',
            'DETAIL_PICTURE' => 'IBLOCK_ELEMENT.DETAIL_PICTURE',
            'PREVIEW_PICTURE' => 'IBLOCK_ELEMENT.PREVIEW_PICTURE',
            $fName
        ];
        $db_res = CIBlockElement::getList(['SORT' => 'ASC'],$filter['filter'], false,false,$filter['select']);
        while ($arId = $db_res->Fetch()){
            $result[] = $arId;
        }
        unset($filter['select']);
        unset($filter['offset']);
        ...
        $filter['filter'] = ['PRODUCT_ID' => $arId];
        $arPrice = \Bitrix\Catalog\PriceTable::getList($filter)->fetchAll();
        foreach ($result as $k => $item) {
            $result[$k]['DETAIL_PICTURE'] = CFile::GetPath($item['DETAIL_PICTURE']);
            $result[$k]['PREVIEW_PICTURE'] = CFile::GetPath($item['PREVIEW_PICTURE']);
            if ($item['ID'] == $arPrice[$k]['PRODUCT_ID'])
                $result[$k]['PRICES'] = $arPrice[$k];
        }
        $result = json_encode($result);
        return $result;
    }
    ...
    public function add()
    {
        ...
        $ImageUrl = $this->requestParams['img'];
        $Image = explode('/', $ImageUrl);
        $Image = $Image[count($Image) - 1];
        $outputFile = $_SERVER['DOCUMENT_ROOT'] . '/upload/tmp/' . $Image;
        file_put_contents($outputFile, file_get_contents($ImageUrl));
        $el = new CIBlockElement;
        $arLoadProductArray = $this->requestParams['ELEMENT'];
